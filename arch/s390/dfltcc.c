/* dfltcc.c - IBM Z DEFLATE CONVERSION CALL support. */

/*
   Use the following commands to build zlib-ng with DFLTCC support:
...
/*
   Compress.
 */
static inline int dfltcc_are_params_ok(int level, uInt window_bits, int strategy, uLong level_mask)
{
...
again:
    ...
    if (!no_flush) {
        /* We need to close a block */
        if (param->bcc) {
            /* We have already asked the hardware to do this */
        } else if (strm->avail_in == 0)
            /* There is no input data, so the hardware will not nohor our
             * request. Close the block manually.
             */
            soft_bcc = 1;
        else
            /* The hardware will honor our request. Set Block Closing Control
